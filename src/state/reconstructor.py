...
class StateReconstructor:
    ...
    def reconstruct_positions(
        self, positions_df: pd.DataFrame, collateral_df: pd.DataFrame
    ) -> List[Position]:
        """
        ...
        if (
            "collateral" in collateral_df.columns
            and "collateral_assets" not in collateral_df.columns
        ):
            collateral_df = collateral_df.rename(
                columns={"collateral": "collateral_assets"}
            )
        ...
        logger.info(
            f"Using prices: {self.pool_config['collateral']}=${collateral_price:.2f}, "
        ...
        for _, row in merged.iterrows():
            try:
                ...
                debt_amount_raw = float(
                    row.get("active_borrow_assets", row.get("active_borrow_assets", 0))
                )

                # Normalize to human-readable units by dividing by 10^decimals
                collateral_amount = collateral_amount_raw / (
                    10**self.collateral_decimals
                )
                ...
                logger.warning(
                    f"Error processing position for {row.get('borrower')}: {e}"
                )
                continue
        ...
        if positions:
            ...
            logger.info(
                f"  Healthy positions: {healthy}/{len(positions)} "
                f"({healthy/len(positions)*100:.1f}%)"
            )

            finite_hfs = [
                p.health_factor for p in positions if p.health_factor != float("inf")
            ]
            if finite_hfs:
                ...
                logger.info(
                    f"  Health factors: avg={avg_hf:.2f}, min={min_hf:.2f}, max={max_hf:.2f}"
                )
    ...
    def create_snapshot(
        ...
        if not pool_state_df.empty:
            ...
            pool_state_df = pool_state_df.sort_values(
                "call_block_time", ascending=False
            )
        ...
        logger.info(
            f"Snapshot created: {snapshot.num_positions} positions, "
    ...
    def load_snapshot(self, input_path: str) -> PoolSnapshot:
        ...
        for p_data in data["positions"]:
            position = Position(
                ...
                lltv=p_data["lltv"],
                timestamp=(
                    datetime.fromisoformat(p_data["timestamp"])
                    if p_data["timestamp"]
                    else None
                ),
            )
