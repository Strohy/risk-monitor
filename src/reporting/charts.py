...
class ChartGenerator:
    ...
    def generate_health_factor_distribution(
        ...
        filename: str = "health_factor_distribution.png"
    ) -> Path:
        """
        ...
        ax.set_title('Health Factor Distribution', fontsize=14, fontweight='bold', pad=20)
    ...
    def generate_stress_test_cascade(
        ...
        filename: str = "stress_test_cascade.png"
    ) -> Path:
        """
        ...
        debt_at_risk = results_df['debt_at_risk_usd'].values / 1_000_000  # Convert to millions

        # Plot line with area fill
        ax.plot(shocks, debt_at_risk, 'o-', linewidth=3, markersize=8,
                color='#d32f2f', label='Debt at Risk', zorder=3)
        ...
        if threshold_10:
            ax.axvline(x=threshold_10, color='orange', linestyle='--', linewidth=2,
                      alpha=0.7, label=f'10% Pool at Risk ({threshold_10:+.0f}%)')

        if threshold_50:
            ax.axvline(x=threshold_50, color='red', linestyle='--', linewidth=2,
                      alpha=0.7, label=f'50% Pool at Risk ({threshold_50:+.0f}%)')
        ...
        if cliffs:
            for cliff in cliffs:
                ...
                if shock_idx is not None:
                    ax.plot(shocks[shock_idx], debt_at_risk[shock_idx], 'r*',
                           markersize=15, label='Cliff Point' if cliffs[0] == cliff else '',
                           zorder=4)

        # Customize chart
        ax.set_xlabel('Collateral Price Shock (%)', fontsize=12, fontweight='bold')
        ax.set_ylabel('Debt at Risk ($M)', fontsize=12, fontweight='bold')
        ax.set_title('Stress Test: Liquidation Cascade', fontsize=14, fontweight='bold', pad=20)
    ...
    def generate_borrower_concentration(
        ...
        # Add title
        ax.set_title('Borrower Concentration by Debt', fontsize=14, fontweight='bold', pad=20)

        # Add concentration warning if needed
        if top_5_pct > 50:
            fig.text(0.5, 0.02, 'Warning: High concentration risk - Top 5 control majority of debt',
    ...
    def generate_all_charts(
        ...
        charts['concentration'] = self.generate_borrower_concentration(concentration_metrics)
